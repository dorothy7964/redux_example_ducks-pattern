...
추가적으로 mapDispatchToProps 부분에선, 이번엔 액션이 두개가 있습니다. 계속 dispatch 해주기가 조금 귀찮죠? 그러한 경우엔 [**bindActionCreators**](https://redux.js.org/api/bindactioncreators) 라는 함수를 사용하면 조금 더 쉽게 할 수 있습니다.
...
### 액션 생성함수 만들기
우리는 잠시 후에, 액션 생성 함수를 간편하게 만들 수 있게 해주는 redux-actions 의 createAction 이라는 함수를 사용하여 작성해볼건데요, 이는 FSA 규칙을 따르는 액션 객체를 만들어주는데, 이 [**FSA**](https://github.com/redux-utilities/flux-standard-action) 규칙은 읽기 쉽고, 유용하고, 간단한 액션 객체를 만들기 위해서 만들어졌습니다.
